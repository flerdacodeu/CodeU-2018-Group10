...
public class AnagramValidityChecker
{
   ...
   Two sentences are anagrams if each word in the resulting
    ...
    public static boolean isSentenceAnagram(String sentence1, String sentence2, boolean isCaseSensitive)
    {
        ...
        if(wordsOfSentence1.length != wordsOfSentence2.length) return false;
        ...
        for(int i = 0 ; i < wordsOfSentence1.length ; i++)
        {
            for(int j = 0 ; j < wordsOfSentence2.length ; j++)
            {
                ...
                // checks if the current word is anagram of the other, and mark it in isSent2WordAnagramFound in case one is the anagram of the other.
        ...
        // if all words in the second sentence are found as anagrams of words in the first sentence
        // (and both have same amount of words), than the sentences are anagrams of each other,
        // else, they ar'nt.
...
/*    Given two strings, determine if one is an anagram of the other. Two words are anagrams of
    ...
    public static boolean isWordAnagram(String s1, String s2, boolean isCaseSensitive)
    {
        ...
        for(int i = 0 ; i < s1.length() ; i++)
        {
            ...
            if(charsCountInWord.containsKey(c))    charsCountInWord.put(c, charsCountInWord.get(c)+1);
            else charsCountInWord.put(c, 1);
        }
        ...
        {
            char c = s2.charAt(i);
            if (!charsCountInWord.containsKey(c) || charsCountInWord.get(c) == 0) return false;
            else    charsCountInWord.put(c, charsCountInWord.get(c) -1);
        }
